...
class Notifier:
    ...
    async def send(self, message: Message) -> None:
        ...
        if token and chat_id:
            jobs.append(self._telegram(token, chat_id, message.text))
        webhook = os.getenv("DISCORD_WEBHOOK_URL")
        if webhook:
            jobs.append(self._discord(webhook, message.text))
        kakao_token = os.getenv("KAKAO_ACCESS_TOKEN")
        if kakao_token:
            jobs.append(self._kakao(kakao_token, message.text))
        for job in jobs:
            try:
                await job
    ...
    async def _kakao(self, token: str, text: str) -> None:
        template = {
            "object_type": "text",
            "text": text[:1800],
            "link": {"web_url": "https://www.daangn.com", "mobile_web_url": "https://www.daangn.com"},
        }
        response = await self.client.post(
